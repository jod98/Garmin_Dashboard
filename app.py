...
def fetch_planned_sessions_live(_client, start_date, end_date):
    """
    Fetches scheduled workout activities directly from Garmin Connect Calendar 
    strictly between start_date and end_date (Monday to Sunday of current week).
    No database calls, no workout library fallbacks.
    """
    sessions = []

    try:
        # Fetch current month's calendar entries
        ...
        # If week crosses into next month, fetch next month's items as well
        ...
        for item in items:
            ...
            # Extract the scheduled date from the Garmin calendar entry
            ...
            # Strict filter: Only keep items scheduled between Mon & Sun of this week
    ...
    if not planned_sessions:
        ...
        for s in planned_sessions:
            ...
            logs_html += (
                f'<div class="activity-card">'
                f'<div class="activity-date">{date_label}</div>'
                f'<div class="activity-metrics"><strong>{title}</div>'
                f'</div>'
            )
        logs_html += "</div>"
        st.markdown(logs_html, unsafe_allow_html=True)
